	...
	for rw.off < end {
		...
		case gap.Ok():
			...
			if fillCache {
				...
				optMR := gap.Range()
				_, err := rw.d.inode.cache.Fill(rw.ctx, reqMR, rw.d.inode.fillRangeLocked(reqMR, optMR), rw.d.inode.size.Load(), mf, pgalloc.AllocOpts{
...
func (d *dentry) Translate(ctx context.Context, required, optional memmap.MappableRange, at hostarch.AccessType) ([]memmap.Translation, error) {
	...
	_, cerr := d.inode.cache.Fill(ctx, required, d.inode.fillRangeLocked(required, optional), d.inode.size.Load(), mf, pgalloc.AllocOpts{
		Kind:    usage.PageCache,
		MemCgID: memCgID,
		Mode:    pgalloc.AllocateAndWritePopulate,
	}, h.readToBlocksAt)
